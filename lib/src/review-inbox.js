...
class ReviewInbox extends Jobs {
  plans;
  constructor(dataDir, plans) {
    super(null, dataDir);
    this.plans = plans;
  }
  async submit({ requestId, summary, checks = [], planId, taskId, kind = "review" }, { sessionId, cwd } = {}) {
    if (!/^[a-zA-Z0-9_-]{1,80}$/.test(requestId || "") || !nonempty(summary, 2e3) || !["review", "task"].includes(kind) || !Array.isArray(checks) || kind === "review" && checks.length < 1 || kind === "task" && checks.length !== 0 || checks.length > 8 || !checks.every((x) => nonempty(x, 500)) || Boolean(planId) !== Boolean(taskId))
      fail(
        "INVALID_REQUEST",
    ...
    return this.exclusive(id, async () => {
      ...
      const assigned = (await this.plans.list()).flatMap(
      ...
      if (planId) {
        ...
        if (plan.state !== "running" || !task || !["running", "waiting_user"].includes(task.state) || task.execution?.sessionId !== sessionId || await realpath(task.worktree.path) !== cwd)
          fail(
            "NOT_PLAN_WORKER",
  ...
  async list({ cwd }) {
    ...
    return requests.filter((x) => x.commonDir === commonDir).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
  }
  async status({ id }) {
    return this.read(id);
  }
  async respond({ id, accepted, feedback, evidence }) {
    if (typeof accepted !== "boolean" || !nonempty(feedback, 2e3) || !Array.isArray(evidence) || !evidence.length || evidence.length > 8 || !evidence.every((x) => nonempty(x, 500)))
      fail(
        "INVALID_REVIEW",
    ...
    return this.exclusive(id, async () => {
      ...
      if (item.response) {
        if (item.response.accepted === accepted && item.response.feedback === feedback && digest(item.response.evidence) === digest(evidence))
          return item;
        fail(
          "REVIEW_CONFLICT",
