...
const defaultSocket = () => process.env.DSH_CODEX_BRIDGE_SOCKET || join(
  process.env.DSH_HOME || join(homedir(), ".dsh"),
  "codex-bridge",
  "bridge.sock"
);
async function request(method, params = {}, { socketPath = defaultSocket(), timeoutMs = 3e4 } = {}) {
...
async function listen(socketPath, invoke) {
  ...
  if (existing) {
    ...
    try {
      await request("hello", {}, { socketPath, timeoutMs: 1e3 });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      for await (const chunk of req) {
        bytes += chunk.length;
        if (bytes > 1e6) throw new Error("Request too large");
        chunks.push(chunk);
      }
  ...
  await new Promise((resolve, reject) => {
    ...
    server.listen(socketPath, () => resolve(void 0));
  });
