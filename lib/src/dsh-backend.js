...
class DshBackend {
  ...
  async create(sessionId, cwd) {
    ...
    if (!inspection.meta?.cwd || await realpath(inspection.meta.cwd) !== await realpath(cwd))
      throw new Error(
  ...
  async inspect(sessionId) {
    ...
    const decided = new Set(
      inspection.events.filter((e) => e.type === "approval/decided").map((e) => e.data.id)
    );
    const pendingApprovals = inspection.events.filter((e) => e.type === "approval/asked" && !decided.has(e.data.id)).map((e) => e.data);
    const completedCalls = new Set(
      inspection.events.filter((e) => e.type === "tool/result").map((e) => e.data.message?.callId)
    );
    const pendingQuestions = inspection.events.filter(
      (e) => e.type === "tool/call" && e.data?.name === "ask_user_question" && !completedCalls.has(e.data.callId)
...
function outcome(events, requestId) {
  const input = events.find(
    (e) => e.type === "user/message" && e.data?.source?.rpcId === requestId
  );
  if (!input) return null;
  const start = events.filter((e) => e.type === "turn/start" && e.seq <= input.seq).at(-1);
  ...
  const text = messages.flatMap((e) => e.data.message?.content || []).filter((x) => x.type === "text").map((x) => x.text).join("\n");
  return {
    turn,
    text,
