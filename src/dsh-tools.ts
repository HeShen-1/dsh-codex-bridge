...
export function registerDshTools(ctx, inbox, plans) {
  ...
  ctx.tools.register({
    ...
    description:
      "Send a new task or question from this DSH session to the durable Codex inbox. Codex must actively read and decide whether to handle it; this does not wake a desktop task or authorize work. Use a stable requestId on retries. A bridge-managed worker must include its planId and taskId.",
    parameters: {
      type: "object",
      properties: {
        requestId: { type: "string" },
  ...
  for (const name of ["check_codex_review", "check_codex_request"])
    ...
    name,
    description:
      "Read Codex's response to a request from this same DSH session. A task request accepted by Codex means it was taken up, not that code passed review. Managed plan reviews follow plan.review and its repair limits.",
    parameters: {
      type: "object",
    ...
    async execute(args, exec) {
      ...
      if (
        ...
        (await realpath(item.cwd)) !== (await realpath(source.cwd))
      )
        fail(
          "NOT_REQUEST_OWNER",
      ...
      if (item.planId && item.kind !== "task") {
        ...
        const review =
          task?.reviews?.find((r) => r.revision === item.planRevision) || null;
        return JSON.stringify({
          id: item.id,
          state: review
            ? review.accepted
