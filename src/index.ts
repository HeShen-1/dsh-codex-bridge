...
export async function apply(ctx: any, config: { socketPath?: string; dataDir?: string; maxParallel?: number; build?: string } = {}) {
  ...
  const browserRequest = async (endpoint: string, payload: any) => {
    try {
      if (!payload || typeof payload !== "object" ||
          !/^[a-zA-Z0-9_-]{1,100}$/.test(payload.sessionId || ""))
      ...
      if (endpoint === "snapshot") {
        ...
        const related = allPlans.filter((p) => p.cwd === cwd ||
          p.tasks?.some((t) => t.worktree?.path === cwd));
        return { ok: true, value: {
          session: { id: payload.sessionId, cwd,
          ...
          plans: related.map((p) => ({
            ...
            tasks: p.tasks.map((t) => ({ id: t.id, title: t.title,
              state: t.state, sessionId: t.execution?.sessionId || null,
      ...
      return { ok: false, error: {
        code: error.code || "BRIDGE_ERROR", message: error.message, details: {},
      } };
    }
  };
    ...
    async fetch(request: Request) {
      try {
        ...
        return Response.json({ ok: false, error: {
          code: "INVALID_REQUEST", message: error.message, details: {},
        } }, { status: 400 });
      }
    },
  }));
