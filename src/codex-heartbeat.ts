...
export class CodexHeartbeat extends Jobs {
  async key(cwd: string) {
  ...
  async getStatus({ cwd }: { cwd: string }) {
  ...
  async report({ cwd, threadId, taskStatus }: {
    cwd: string; threadId: string; taskStatus: string;
  }) {
    if (!/^[0-9a-f-]{36}$/i.test(threadId || "") ||
        !["active", "idle", "needs_attention", "unknown"].includes(taskStatus))
