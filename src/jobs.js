...
export async function git(cwd, ...args) {
  return (
    await exec("git", ["-C", cwd, ...args], { maxBuffer: 8_000_000 })
  ).stdout.trim();
}
...
export class Jobs {
  ...
  async submit({ id, cwd, prompt, sourceTask = "" }) {
    this.file(id);
      ...
      if (result?.finished && !inspection.running)
        job.state =
          job.state === "stop_requested"
            ? "stopped"
            : result.reason?.kind === "completed"
              ? "submitted"
              : "failed";
      else if (inspection.running) {
        if (job.state !== "stop_requested")
          job.state =
            job.pendingApprovals.length || job.pendingQuestions.length
              ? "waiting_user"
              : "running";
