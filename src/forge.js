...
export async function command(bin, args, cwd) {
  return (
    await exec(bin, args, {
      cwd,
      timeout: 30000,
...
export class Forge {
  ...
  async validate(plan) {
    ...
    if (
      !["github", "gitlab"].includes(d.forge) ||
      !["origin", ...(await git(plan.cwd, "remote")).split("\n")].includes(
        d.remote,
      ) ||
      !d.repository ||
      !d.targetBranch ||
  ...
  async findIssues(plan, marker) {
    ...
    return rows
      .filter((x) => (x.body || x.description || "").includes(marker))
      .map((x) => ({ number: x.number ?? x.iid, url: x.url ?? x.web_url }));
  }
  ...
  async issue(plan, task, persist) {
    ...
    const d = plan.delivery,
      marker = `dsh-bridge:${plan.id}:${task.id}`;
    ...
    const output = await this.withBody(body, (path) =>
      this.run(
    ...
    const url =
      output.match(/https?:\/\/[^\s]+\/(?:issues|issues\/)\/??\d+/)?.[0] ||
      output.match(/https?:\/\/[^\s]+\/\d+/)?.[0];
    if (!url)
      fail(
        "REMOTE_UNKNOWN",
  ...
  async deliver(plan, persist) {
    if (
      !["reviewed", "delivery_unknown", "awaiting_merge"].includes(plan.state)
    )
      fail(
        "NOT_REVIEWED",
    ...
    if (
      (await git(cwd, "rev-parse", "HEAD")) !== head ||
      (await changed(cwd)).length
    )
      fail(
        "WORKTREE_CHANGED",
    ...
    // Fail before pushing if target has advanced; integration must cover its exact current history.
    ...
    const remoteHead = (
      await git(cwd, "ls-remote", "--heads", d.remote, `refs/heads/${branch}`)
    ).split(/\s/)[0];
